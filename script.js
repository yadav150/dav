...
// ===== SEARCH DROPDOWN =====
(function(){'use strict';var q=[{name:'Age Calculator',path:'/dav/tools/age-calculator/index.html'},{name:'Loan Calculator',path:'/dav/tools/loan-calculator/index.html'},{name:'GST Calculator',path:'/dav/tools/gst-calculator/index.html'}];var a=[{name:'Age Calculator',path:'/dav/tools/age-calculator/index.html'},{name:'SIP Calculator',path:'/dav/tools/sip-calculator/index.html'},{name:'PDF Editor',path:'/dav/tools/pdf-editor/index.html'},{name:'Image Resizer',path:'/dav/tools/image-resizer/index.html'},{name:'Text Counter',path:'/dav/tools/text-counter/index.html'},{name:'Unit Converter',path:'/dav/tools/unit-converter/index.html'},{name:'Password Generator',path:'/dav/tools/password-generator/index.html'},{name:'QR Generator',path:'/dav/tools/qr-generator/index.html'},{name:'GST Calculator',path:'/dav/tools/gst-calculator/index.html'},{name:'Number Converter',path:'/dav/tools/number-converter/index.html'},{name:'Percentage Calculator',path:'/dav/tools/percentage-calculator/index.html'},{name:'BMI Calculator',path:'/dav/tools/bmi-calculator/index.html'},{name:'Loan Calculator',path:'/dav/tools/loan-calculator/index.html'},{name:'Accessibility Tool',path:'/dav/tools/accessibility-tool/index.html'}];var c=document.querySelector('.search-container');if(!c)return;var h='';h+='<div class="search-wrapper">';h+='<div class="search-box">';h+='<svg class="search-icon" viewBox="0 0 24 24" fill="none" stroke="#1a5c3a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>';h+='<input type="text" id="searchInput" placeholder="Search..." autocomplete="off">';h+='</div>';h+='<div class="search-dropdown" id="searchDropdown">';h+='<div class="dropdown-section">';h+='<span class="dropdown-label">Quick Tools</span>';h+='<ul class="dropdown-list" id="quickToolsList">';for(var i=0;i<q.length;i++){h+='<li><a href="'+q[i].path+'">'+q[i].name+'</a></li>'}h+='</ul></div>';h+='<div class="dropdown-divider"></div>';h+='<div class="dropdown-section">';h+='<span class="dropdown-label">All Tools</span>';h+='<ul class="dropdown-list" id="allToolsList">';for(var i=0;i<a.length;i++){h+='<li><a href="'+a[i].path+'">'+a[i].name+'</a></li>'}h+='</ul></div></div></div>';c.innerHTML=h;var s=document.getElementById('searchInput');var d=document.getElementById('searchDropdown');var al=document.getElementById('allToolsList');s.addEventListener('focus',function(){d.classList.add('open')});document.addEventListener('click',function(e){if(!c.contains(e.target)){d.classList.remove('open')}});s.addEventListener('keydown',function(e){if(e.key==='Escape'){d.classList.remove('open');s.blur()}});s.addEventListener('input',function(){var v=this.value.toLowerCase().trim();var items=al.querySelectorAll('li');var qs=document.getElementById('quickToolsList');var qsec=qs?qs.closest('.dropdown-section'):null;if(v===''){items.forEach(function(item){item.style.display='block'});if(qsec)qsec.style.display='block';d.classList.add('open');return}if(qsec)qsec.style.display='none';var has=false;items.forEach(function(item){var t=item.textContent.toLowerCase();if(t.indexOf(v)!==-1){item.style.display='block';has=true}else{item.style.display='none'}});var nr=document.getElementById('noResult');if(has){if(nr)nr.style.display='none'}else{if(!nr){var li=document.createElement('li');li.id='noResult';li.textContent='No tools found';li.style.cssText='padding:10px 16px;color:#4a6b5a;font-size:.9rem;text-align:center;';al.appendChild(li)}else{nr.style.display='block'}}d.classList.add('open')});d.addEventListener('click',function(e){var link=e.target.closest('a');if(link){d.classList.remove('open');s.value=''}})})();
